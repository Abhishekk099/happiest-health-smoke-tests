@cms @EditorsPick
Feature: EditorsPick Validation

  @EditorsPickEndToEndValidation
  Scenario Outline: Verify user select Mark this post as Editors Pick for an Recorded Webinar
...
#    And User click on "wp-submit" button using "id"
    ...
    And User enters values in "Webinar Speakers" input field
      | Dr Amrish Mithal |
      | Dr Anoop Misra |
    And User selects "Show this on Hero Section (Main Slider)" checkbox in CMS page
    And User selects "Show this on Category Hero Section" checkbox in CMS page
    And User selects "Show this on Breaking News Section" checkbox in CMS page
    And User selects "Mark this post as Editors Pick" checkbox in CMS page
    ...
    And User click on "/editors-pick-2" link using href attribute
    #And User navigates to window "Editor's Pick | Happiest Health"
    Then User Verify "<Title>" Link is displayed
    And User clicks on "<Categories>" Link using linkName
    And User click on "<Category>" text
    #And User navigates to window "Ageing Archives - Happiest Health"
    #Webminar is not displaying in hero section
    #Then User verify "<Title>" is displayed in Hero Section
    Then User verify "<Title>" is displayed in Breaking News Section
    Given User has launched CMS URL
    When User enters email value as "<username>"
    And User enters password value as "<password>"
    And User clicks on "<Recorded Webinars>" link
    And User click on "<Title>" edit button
    And User selects "Show this on Hero Section (Main Slider)" checkbox in CMS page
    And User selects "Show this on Category Hero Section" checkbox in CMS page
    And User selects "Show this on Breaking News Section" checkbox in CMS page
    And User selects "Mark this post as Editors Pick" checkbox in CMS page
    ...
    And User click on "/editors-pick-2" link using href attribute
    #And User navigates to window "Editor's Pick | Happiest Health"
    Then User Verify "<Title>" Link is not displayed
    And User clicks on "<Categories>" Link using linkName
    And User click on "<Category>" text
    #And User navigates to window "Ageing Archives - Happiest Health"
    Then User verify "<Title>" is removed in Hero Section
    Then User verify "<Title>" is removed in Breaking News Section
    Given User has launched CMS URL
    When User enters email value as "<username>"
    And User enters password value as "<password>"
    And User clicks on "<Recorded Webinars>" link
